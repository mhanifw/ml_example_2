...
```{r}
# Clean up the above code to make it cleaner. Key functions: logistic_reg(), 
...
# We are concerned if this model is overfitted. 
# Lets do this again with 1% of the data.
```
...
split <- 
  initial_split(data = clean, prop = 1/100)
...
```{r}
# Make model on training

logistic_reg() %>%
  set_engine("glm") %>%
  fit(dvote ~ gender + party + state + office, data = train) %>%
...
```{r}
# Ultimately we want to make models that can fit to new data that it hasnt seen before.
